...
public interface SteamGameTagRepository extends JpaRepository<SteamGameTag,Long>{
    ...
    @Query(value="select s.steam_app_id from steam_game_tags s join game_tags t on t.id=s.tag_id "
            ...
            + "and ((:priceMode is null and (:priceUnrestricted=true or (case when g.is_free=true then 0 else g.price_current end) between :priceMin and :priceMax)) "
            + "or (:priceMode='FREE' and g.is_free=true) or (:priceMode='PAID' and g.is_free is not true and g.price_current is not null and g.price_current between :priceMin and :priceMax)) "
