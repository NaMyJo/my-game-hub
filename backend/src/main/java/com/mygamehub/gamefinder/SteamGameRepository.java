...
public interface SteamGameRepository extends JpaRepository<SteamGame, Long> {
    ...
    @Query(value = "with missing as (select g.igdb_status, "
            ...
            + "('multiplayer','coop','online-coop','local-coop','pvp','online-pvp','massively-multiplayer')) "
            + "or exists (select 1 from steam_game_igdb_terms r join igdb_taxonomy_terms t "
            + "on t.id=r.taxonomy_term_id where r.steam_app_id=g.steam_app_id "
            + "and t.source_type='GAME_MODE' and t.igdb_term_id in (2,3,4,5,6))) as multi_evidence, "
            + "(exists (select 1 from steam_game_tags r join game_tags t on t.id=r.tag_id "
            + "where r.steam_app_id=g.steam_app_id and t.canonical_name='singleplayer') "
            + "or exists (select 1 from steam_game_igdb_terms r join igdb_taxonomy_terms t "
            + "on t.id=r.taxonomy_term_id where r.steam_app_id=g.steam_app_id "
            + "and t.source_type='GAME_MODE' and t.igdb_term_id=1)) as single_evidence "
            + "from steam_games g where g.game_catalog_eligible=true and g.metadata_status='SUCCESS' "
            + "and g.store_type='game' and (g.lifecycle_status is null or g.lifecycle_status='ACTIVE') "
            + "and g.min_players is null and g.max_players is null "
            + "and g.online_max_players is null and g.online_coop_max_players is null) "
            + "select count(*) as \"playerDataMissingTotal\", "
            ...
            + "as \"igdbSuccessSingleplayerOnlyCandidateCount\", "
            + "count(*) filter (where igdb_status='SUCCESS' and not multi_evidence and not single_evidence) "
    ...
    @Query(value = "with evidence as (select g.steam_app_id, g.name, g.igdb_game_id, "
            ...
            + "or exists (select 1 from steam_game_igdb_terms r join igdb_taxonomy_terms t "
            + "on t.id=r.taxonomy_term_id where r.steam_app_id=g.steam_app_id "
            + "and t.source_type='GAME_MODE' and t.igdb_term_id in (2,3,4,5,6))) multi_evidence, "
            + "(exists (select 1 from steam_game_tags r join game_tags t on t.id=r.tag_id "
            + "where r.steam_app_id=g.steam_app_id and t.canonical_name='singleplayer') "
            + "or exists (select 1 from steam_game_igdb_terms r join igdb_taxonomy_terms t "
            + "on t.id=r.taxonomy_term_id where r.steam_app_id=g.steam_app_id "
            + "and t.source_type='GAME_MODE' and t.igdb_term_id=1)) single_evidence "
            + "from steam_games g where g.game_catalog_eligible=true and g.metadata_status='SUCCESS' "
            + "and g.store_type='game' and (g.lifecycle_status is null or g.lifecycle_status='ACTIVE') "
            + "and g.min_players is null and g.max_players is null "
            + "and g.online_max_players is null and g.online_coop_max_players is null), "
            + "classified as (select e.*, case when multi_evidence then 'MULTIPLAYER_CANDIDATE' "
            + "when single_evidence then 'SINGLEPLAYER_ONLY_CANDIDATE' else 'UNKNOWN' end classification "
            + "from evidence e) select c.steam_app_id as \"steamAppId\", c.name, "
            ...
            + "coalesce((select string_agg(distinct coalesce(nullif(t.slug,''),t.name), '|' "
            + "order by coalesce(nullif(t.slug,''),t.name)) from steam_game_igdb_terms r "
            + "join igdb_taxonomy_terms t on t.id=r.taxonomy_term_id "
            + "where r.steam_app_id=c.steam_app_id and t.source_type='GAME_MODE'),'') "
            + "as \"igdbGameModes\" from classified c where c.classification=:classification "
            + "order by c.steam_app_id", nativeQuery = true)
    List<PlayerMissingSampleProjection> findPlayerMissingSamples(
