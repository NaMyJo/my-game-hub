...
public class GameFinderPreferenceService {
 ...
 @Transactional public GameFinderPreferenceResponse save(String uid,GameFinderPreferenceRequest body){
  ...
  var p=preferences.findById(uid).orElseGet(()->new GameFinderUserPreference(uid)); p.update(body.selectedSteamAppIds(),tags,body.priceMin(),body.priceMax(),body.includeAdult(),body.playerMin(),body.playerMax(),body.releasePreference()); preferences.save(p);
 ...
 private GameFinderPreferenceResponse response(GameFinderUserPreference p,List<Long> selected,List<Long> recent){
  ...
  return new GameFinderPreferenceResponse(items(selected,byId),p==null?List.of():p.tags(),p==null?0:p.getPriceMin(),p==null?100000:p.getPriceMax(),p!=null&&p.isIncludeAdult(),p==null?1:p.getPlayerMin(),p==null?15:p.getPlayerMax(),p==null?ReleasePreference.RECENT:p.getReleasePreference(),items(recent,byId));
 }
